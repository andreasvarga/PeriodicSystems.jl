...
function ps2ls(psys::PeriodicStateSpace{<:AbstractPeriodicArray{:d,T}}, kstart::Int = 1; ss::Bool = false, cyclic::Bool = false) where {T}
    ...
    copyto!(view(B,1:n,M-m+1:M),psys.B[kstart-1])
    i1 = n+1
    j1 = 1
    pbc = pb
    for i = 1:nb
        ...
        for j = 1:pbc
            ...
            copyto!(view(B,i1:i2,j1:j2),psys.B[k])
            k += 1
            i1 = i2+1
            j1 = j2+1 
        end
    ...
    for i = 1:nc
        ...
        for j = 1:pc
            ...
            copyto!(view(C,i1:i2,j1:j2),psys.C[k]) 
            k += 1
            i1 = i2+1
            j1 = j2+1 
        end
    ...
    for i = 1:nd
        ...
        for j = 1:pd
            ...
            copyto!(view(D,i1:i2,j1:j2),psys.D[k])
            k += 1
            i1 = i2+1
            j1 = j2+1 
        end
    ...
    copyto!(view(A,1:n,N-nf+1:N),psys.A[kstart-1])
    i1 = n+1
    j1 = 1
    pac = pa
    U = -I
    for i = 1:na
        ...
        for j = 1:pac
            ...
            copyto!(view(A,i1:i2,j1:j2),psys.A[k])
